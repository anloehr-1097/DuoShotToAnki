...
std::string OrgFormatter::format_header(const std::string& header, int level) const {
...
std::string OrgFormatter::format_image_link(const std::string& image_link) const {
    return "# [[" + image_link + "]]\n\n";
}
...
void OrgFormatter::append_card(const DuoAnkiResponse& card, const std::string& image_path) const {
    ...
    if (std::filesystem::exists(org_file_path)) {
        ...
        if (in.fail()) {
            std::cerr << "Failed to open " << org_file_path << " for reading." << std::endl;
            return;
        }
        ...
        std::string english_marker = format_header("Front", 3) + card.english + "\n";
    ...
    if (is_duplicate) {
        std::cout << "Duplicate card detected for: " << card.english << ". Skipping." << std::endl;
    ...
    if (out.fail()) {
        std::cerr << "Failed to open " << org_file_path << " for appending." << std::endl;
        return;
    }
    ...
    out << format_header(card.card_name, 2) << add_properties() << format_header("Front", 3)
        << card.english << "\n"
        << format_header("Back", 3) << card.russian << "\n"
        << format_image_link(image_path);
    out.close();
}
