...
private:
    ...
    std::string format_header(const std::string&& header, int level = 1) const;
    std::string format_image_link(const std::string& image_link) const;
    std::string add_properties() const;

public:
    OrgFormatter(const std::filesystem::path org_file_path, const std::filesystem::path image_path);
    void append_card(const DuoAnkiResponse& card_info, const std::string& image_path) const;
};
